...
type MACDPoint = { time: string; macd: number; signal: number; histogram: number };
...
export default function MACDChart({ data, priceData, timeframe, interval, theme }: Props) {
  ...
  useEffect(() => {
    ...
    // Histogram (rendered first so lines draw on top)
    ...
    // Zero reference
    macdSeries.createPriceLine({ price: 0, color: 'rgba(148,163,184,0.3)', lineWidth: 1, lineStyle: 2, title: '' });
  ...
  return (
    <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 overflow-hidden">
      <div className="flex items-center gap-3 px-4 py-2 border-b border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800">
        <span className="text-xs font-semibold text-slate-600 dark:text-slate-300">MACD (12, 26, 9)</span>
        <div className="flex items-center gap-3 ml-1">
          <span className="flex items-center gap-1.5 text-[10px] text-slate-400 dark:text-slate-500">
            <span className="inline-block w-4 h-0.5 bg-sky-400 rounded" />MACD
          </span>
          <span className="flex items-center gap-1.5 text-[10px] text-slate-400 dark:text-slate-500">
            <span className="inline-block w-4 h-0.5 bg-orange-400 rounded" />Signal
          </span>
        </div>
      </div>
      <div className="p-2">
